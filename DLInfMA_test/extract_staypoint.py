...
def write2csv(ans, save_path):
    ...
    for sp in ans:
        ...
        plan_no = sp.plan_no
        dri_id = sp.dri_id
        sp_list.append([plan_no, dri_id, lng, lat, start, duration])
...
if __name__ == '__main__':
    t0 = time.time()
    # 用停留距离和停留时间去判断 读取轨迹数据
    path = '/Volumes/T7/destination_modify'
    filename = 'traj_flow_rizhao.csv'
    traj_data = pd.read_csv(os.path.join(path, filename))

    # 读取运单数据
    waybill_path = '/Volumes/T7/destination_modify/waybill_data_rizhao.csv'
    waybill_data = pd.read_csv(waybill_path)
    t1 = time.time()
    print(t1-t0, 's')
    ...
    for plan_no, plan_no_traj in tqdm(traj_data.groupby('plan_no')):
        traj_list = []
        ans_staypoint = []
        dri_id = waybill_data[waybill_data['plan_no'] == plan_no]['driver_id'].values[0]
        ...
        for index, row in plan_no_traj.iterrows():
            ...
            traj_list.append(TrajPoint(plan_no, lng, lat, time))
        ...
        if len(traj_segments) != 0:
            for traj_segment in traj_segments:
                ...
                sp = StayPoint(cen_lng=cen_lng, cen_lat=cen_lat, start_staytime=start, duration=duration,
                               plan_no=plan_no, dri_id=dri_id)
                staypoint_all_vis.append(sp)
    save_path = '/Volumes/T7/destination_modify'
    save_filename = 'sp_rizhao.csv'

...
def load_rn_shp(path, is_directed=True):
    ...
    edge_idx_file = os.path.join(os.getcwd(), 'data/edge_idx_file', cluster_id)
    if not os.path.exists(edge_idx_file):
        try:
            os.makedirs(edge_idx_file)
        except:
            print(edge_idx_file)
    np.save(edge_idx_file+'/edge_idx.npy', edge_idx)
    if not is_directed:
